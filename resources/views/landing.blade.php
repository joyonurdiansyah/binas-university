...
<div
    ...
    <div class="flex gap-16 overflow-x-auto">
        @if ($cooperationImg->isEmpty())
            <p class="text-lg text-center text-neutral-200 font-montserrat">
                Data gambar tidak tersedia
            </p>
        @else
            @foreach ($cooperationImg as $image)
                <img class="w-12 md:w-fit" src="{{ asset('storage/' . $image->image) }}/assets/images/Tokyo-Daigaku.png"
                    alt="Cooperation" />
            @endforeach
        @endif
    </div>
    ...
    <div class="container grid items-center grid-cols-1 gap-12 md:grid-cols-2">
        <div class="space-y-4 lg:ml-20">
            ...
            @if (empty($abouts->content) && empty($abouts->image))
                <p class="text-lg font-semibold font-montserrat sm:text-xl text-neutral-200">
                    No data available
                </p>
            @else
    ...
    <div class="grid grid-cols-2 gap-12 text-center lg:grid-cols-4 mt-11">
        @if ($rectors->isEmpty()) <!-- Check if the collection is empty -->
            <div class="col-span-2 lg:col-span-4">
                <p class="text-lg text-neutral-200 font-montserrat">
                    No data available
                </p>
            </div>
        @else
            @foreach ($rectors as $rektor)
                <div class="flex flex-col items-center">
                    <div class="mb-6 overflow-hidden rounded-full w-fit">
                        <img src="{{ asset('storage/' . $rektor->image) }} /assets/images/orang-3.png"
                            alt="{{ $rektor->nama }}" />
                    </div>
